...
class C_LinearTransform:
	...
	def __init__(self, far=10., near=0.3, angleOfView = 90.0):
		self.angleOfView = angleOfView;
		self.near = near;
		self.far = far;
		self.factor = 100.
		
		self.S = 1 / (np.tan((angleOfView / 2.0) * (np.pi / 180.0)));
	...
	def perspectiveProjection(self, T_M, t_point):
		...
		r_3_3 = (-2 / (self.far - self.near))
		r_3_4 = -(self.far + self.near) / (self.far - self.near)  
		...
		return point.C_Point((T_AFTER[0][0] / T_AFTER[2][0])*self.factor, (T_AFTER[1][0] / T_AFTER[2][0])*self.factor, T_AFTER[2][0])
