...
class C_Vector(line.C_Line):
	def __init__(self, p_a, p_b, color=(0, 0, 0), name='none', has_text=False):
	...
	def render(self, c_draw):
		...
		if(self.has_text):
			coordenateText = global_var.myfont.render(f'({self.p_a.getNormalizedX():0.1f}, {self.p_a.getNormalizedY():0.1f}, {self.p_a.getNormalizedZ():0.1f})', 1, (0, 0, 0))
			c_draw.screen.blit(coordenateText, (int(P_A.getScreenX() + c_draw.SCREEN_WIDTH / 2) + 8,  int(P_A.getScreenY() + c_draw.SCREEN_HEIGHT / 2)))
